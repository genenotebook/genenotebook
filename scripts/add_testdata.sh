...
node scripts/add_reference.js -u admin -p admin -r test testdata/testdata.fasta &&\

echo "adding test annotation" &&\
node scripts/add_annotation.js -u admin -p admin -r test -t test testdata/testdata.gff &&\
