...
if (Meteor.isServer) {
  ...
  class JobCollection extends share.JobCollectionBase {
    ...
    constructor(root, options) {
      ...
      for (let level of Array.from(this.ddpPermissionLevels.concat(this.ddpMethods))) {
        this.allows[level] = [];
        this.denys[level] = [];
      }
    ...
    _methodWrapper(method, func) {
      ...
      const permitted = (userId, params) => {
        const performTest = tests => {
          let result = false;
          for (var test of Array.from(tests)) {
            if (result === false) {
              result = result || (() => { switch (myTypeof(test)) {
        ...
        const performAllTests = allTests => {
          let result = false;
          for (let t of Array.from(this.ddpMethodPermissions[method])) {
            if (result === false) {
    ...
    promote(milliseconds) {
      if (milliseconds == null) { milliseconds = 15*1000; }
