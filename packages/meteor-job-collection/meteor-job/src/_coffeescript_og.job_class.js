...
(function () {
  ...
  // Exports Job object
  let Job; let JobQueue; let _clearInterval; let _setImmediate; let _setInterval; let concatReduce; let isBoolean; let isFunction; let isInteger; let isNonEmptyString; let isNonEmptyStringOrArrayOfNonEmptyStrings; let methodCall; let optionsHelp; let reduceCallbacks; let splitLongArray;
  const { splice } = [];
  const { indexOf } = [];
  ...
  methodCall = function (root, method, params, cb, after = (function (ret) {
    ...
    let apply; let name; let ref; let ref1; let ref2; let
      ref3;
  ...
  splitLongArray = function (arr, max) {
    let i; let k; let ref; let
      results;
    if (!(arr instanceof Array && max > 0)) {
      throw new Error('splitLongArray: bad params');
    }
    results = [];
    for (i = k = 0, ref = Math.ceil(arr.length / max); (ref >= 0 ? k < ref : k > ref); i = ref >= 0 ? ++k : --k) {
  ...
  JobQueue = class JobQueue {
    constructor(root1, type1, ...options) {
      let ref; let ref1; let ref2; let ref3; let ref4; let
        worker;
    ...
    _failJobs(tasks, callback) {
      let count; let job; let k; let len; let
        results;
    ...
    resume() {
      ...
      for (w = k = 1, ref = this.concurrency; (ref >= 1 ? k <= ref : k >= ref); w = ref >= 1 ? ++k : --k) {
        _setImmediate(this._process.bind(this));
      }
      return this;
    }
  ...
  Job = (function () {
    ...
    class Job {
      ...
      static setDDP(ddp = null, collectionNames = null, Fiber = null) {
        let collName; let k; let len; let
          results;
      ...
      static getJobs(root, ids, ...options) {
        let cb; let chunkOfIds; let chunksOfIds; let k; let len; let myCb; let ref; let
          retVal;
        ref = options, [...options] = ref, [cb] = splice.call(options, -1);
        [options, cb] = optionsHelp(options, cb);
        if (options.getLog == null) {
