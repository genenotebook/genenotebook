...
const EmailAddress = ({ emails, onChange, editing }) => emails.map((email, i) => {
  // start counting at 1
  const index = i + 1;
  return (
...
class UserProfile extends React.Component {
  ...
  saveChanges = () => {
    const {
      _id: userId, roles, profile, emails,
    } = this.state;
    updateUserInfo.call({
      userId, roles, profile, emails,
    }, (err, res) => {
      if (err) alert(err);
    });
  };
  ...
  render() {
    const {
      username, profile, emails, resetPassword, editing, roles,
    } = this.state;
