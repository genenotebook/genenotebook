...
Tinytest.add('JobCollection default constructor', function(test) {
  ...
  if (Meteor.isServer) {
    ...
    return test.equal(Object.keys(defaultColl.denys).length, 22, "denys not properly initialized");
  } else {
    return test.equal(defaultColl.logConsole, false, "Doesn't have a logConsole");
  }
});
...
if (Meteor.isServer) {
  ...
  Tinytest.add('Set permissions to allow admin on ClientTest', test => test.equal(clientTestColl.allows.admin[0](), true));

  Tinytest.add('Set polling interval', function(test) {
    ...
    return test.notEqual(interval, serverTestColl.interval, "serverTestColl interval not updated");
  });
}
...
if (Meteor.isServer) {
  ...
  Tinytest.addAsync('Create a server-side job and see that it is added to the collection and runs', function(test, onComplete) {
    let ev;
...
Tinytest.addAsync('Create a job and see that it is added to the collection and runs', function(test, onComplete) {
  const jobType = `TestJob_${Math.round(Math.random()*1000000000)}`;
  const job = new Job(testColl, jobType, { some: 'data' });
  test.ok(validJobDoc(job.doc));
