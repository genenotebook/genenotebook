...
class JobCollectionBase extends Mongo.Collection {
  ...
  constructor(root, options) {
    ...
    this._createLogEntry = function(message, runId = null, level, time, data = null) {
      if (message == null) { message = ''; }
      if (level == null) { level = 'info'; }
      if (time == null) { time = new Date(); }
      const l = { time, runId, message, level };
      return l;
    };
  ...
  _idsOfDeps(ids, antecedents, dependents, jobStatuses) {
    ...
    if (antecedents) {
      ...
      ).forEach(function(d) { if (!Array.from(antsArray).includes(i)) { return (() => {
        const result = [];
        for (let i of Array.from(d.depends)) {           result.push(antsArray.push(i));
        }
        return result;
      })(); } });
    ...
    if (dependsQuery.length > 0) {
      this.find(
        ...
        if (!Array.from(dependsIds).includes(d._id)) { return dependsIds.push(d._id); }
      });
    }
    return dependsIds;
  }
  ...
  _rerun_job(doc, repeats, wait, repeatUntil) {
    ...
    let jobId, logObj;
    if (repeats == null) { repeats = doc.repeats - 1; }
    if (wait == null) { wait = doc.repeatWait; }
    if (repeatUntil == null) { ({ repeatUntil } = doc); }
    const id = doc._id;
    const { runId } = doc;
    const time = new Date();
    delete doc._id;
      ...
      if (num > 0) {
        ...
        if ((foundDocs != null ? foundDocs.length : undefined) > 0) {
          if (this.scrub != null) {
            foundDocs = ((() => {
              const result = [];
              for (d of Array.from(foundDocs)) {                 result.push(this.scrub(d));
              }
              return result;
            })());
          }
