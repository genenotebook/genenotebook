...
const isNonEmptyStringOrArrayOfNonEmptyStrings = (sa) => isNonEmptyString(sa)
   || (sa instanceof Array
   && (sa.length !== 0)
   && (((() => {
     const result = [];
...
class JobQueue {
  constructor(root, type, ...rest) {
    ...
    this.pollInterval = (options.pollInterval != null) && !options.pollInterval
      ? Job.forever
    ...
    if (!this._getWorkOutstanding && !this.paused) {
      ...
      if (numJobsToGet > 0) {
        ...
        return Job.getWork(this.root, this.type, options, (err, jobs) => {
          ...
          } if ((jobs != null) && jobs instanceof Array) {
  ...
  _failJobs(tasks, callback) {
    ...
    let count = 0;
    return Array.from(tasks).map((job) => job.fail('Worker shutdown', (err, res) => {
      count++;
  ...
  _stop(callback) {
    ...
    return this._stopGetWork(() => {
      ...
      this._tasks = [];
      return this._waitForTasks(() => this._failJobs(tasks, callback));
    });
  }

  _soft(callback) {
    return this._stopGetWork(() => this._waitForTasks(callback));
  }
  ...
  shutdown(...args) {
    const adjustedLength = Math.max(args.length, 1); let options = args.slice(0, adjustedLength - 1); let
...
Job = (function() {
  class Job {
    ...
    static setDDP(ddp = null, collectionNames = null, Fiber = null) {
      ...
      return Array.from(collectionNames).map((collName) => {
        if ((ddp == null) || (ddp.close == null) || (ddp.subscribe == null)) {
          ...
          if ((ddp === null) && ((typeof Meteor !== 'undefined' && Meteor !== null ? Meteor.apply : undefined) != null)) {
            ...
            return this._setDDPApply(Meteor.apply, collName);
          ...
          return this._setDDPApply(ddp.apply.bind(ddp), collName);
          // result.push(this._setDDPApply(ddp.apply.bind(ddp), collName));
        } else { // This is the npm DDP package
          if (Fiber == null) {
            return this._setDDPApply(ddp.call.bind(ddp), collName);
    ...
    static getWork(root, type, ...rest) {
      const adjustedLength = Math.max(rest.length, 1); let options = rest.slice(0, adjustedLength - 1); let
    ...
    constructor(rootVal, type, data) {
      ...
      if ((typeof doc !== 'object')
        ...
        // This case is used to create local Job objects from DDP calls
        this._doc = doc;
      } else { // This is the normal "create a new object" case
        // console.log({ rootVal, _this: this });
        const time = new Date();
        this._doc = {
          runId: null,
          type,
        ...
        this.priority().retry().repeat().after()
          .progress()
          .depends()
          .log('Constructed');
      }
    ...
    retry(_options = 0) {
      // if (_options == null) { _options = 0; }
      let options;
      if (isInteger(_options) && (_options >= 0)) {
        options = { retries: _options };
      }
      if (typeof options !== 'object') {
    ...
    log(message, ...rest) {
      ...
      this._doc.log.push({
        time: new Date(), runId: null, level: options.level, message,
      });
      if ((cb != null) && (typeof cb === 'function')) {
